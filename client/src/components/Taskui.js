  ...
  return (
    <>
      <div className="task-card">
        <span>
          <button className="btn" id="edit" onClick={editTask}>
            <i class="bi bi-pencil-square"></i>Edit Task
          </button>
        </span>
        <h3 className="task-heading">{props.task.description}</h3>
        <p>
          {" "}
          Status :{" "}
          {props.task.completed === "true" || props.task.completed === "True"
            ? "completed"
            : "pending"}
        </p>
      ...
      <Modal isOpen={modalState}>
        <div className="task-input-modal">
          <form method="PATCH">
            <div className="card-header">Update Task here!</div>
